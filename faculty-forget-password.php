...
//Load Composer's autoloader
  ...
  {
      header("location:admin-forget-pass");
  }
  else{
    
    $select=mysqli_query($con,"select * from faculty_master where f_id='{$_SESSION['ffid']}'") or die(mysqli_error($con));
    ...
    $mail = new PHPMailer(true);                              // Passing `true` enables exceptions
            try {
               ...
               // $mail->addAttachment('/var/tmp/file.tar.gz'); 
                ...
                $mail->send();
                echo "<script>alert('OTP SEND...');window.location='faculty-check-otp.php';</script>";
