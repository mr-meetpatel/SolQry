...
FROM
    ...
    INNER JOIN `oms`.`notification_master` 
        ON (`user_type_master`.`u_id` = `notification_master`.`n_from`) where `notification_master`.`n_to`='Student' and  `notification_master`.`id`='{$_SESSION['sid']}';") or die(mysqli_error($con));
