...
<body class="container">
    ...
    $fname= mysqli_query($con,"select f_name from faculty_master where f_id='{$_GET['fid']}'");
    $fnamedata= mysqli_fetch_array($fname);
    echo $fnamedata['f_name'];
    ?></h2>
<center><a href="faculty-list.php">back</a></center>
...
<script>
    ...
    setInterval(function checkMsg()
    {
        ...
        $.post("checkmsg.php",{
          stu:"<?php echo $_SESSION['sid']?>",
          faculty:"<?php echo $_GET['fid']?>"
        },
...
if(isset($_POST['send']))
{
  ...
  $sql=mysqli_query($con,"INSERT INTO `query_master` ( `from_u_id`, `msg_from`, `to_u_id`, `msg_to`, `query`) VALUES ( '3', '{$from}', '2', '{$to}', '{$msg}');") or die(mysqli_error($con));
}
