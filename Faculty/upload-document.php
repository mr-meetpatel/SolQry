...
$user=mysqli_query($con,"select u_id  from faculty_master where f_id='{$_SESSION['fid']}'") or die(mysqli_error($con));
...
if (isset($_POST['btn'])) {
    ...
    $des="documents/".$_FILES['file']['name'];
    if(!file_exists($des))
    {
        if ( $_FILES['file']['type'] == "application/pdf")
        {
            move_uploaded_file($file, $des);
            $q = mysqli_query($con, "INSERT INTO `document_master` ( `f_id`, `b_id`, `document_file`, `info`, `sem`) VALUES ('{$_SESSION['fid']}', '{$_SESSION['fb_id']}', '{$des}', '{$info}', '{$sem}');") or die(mysqli_error($con));
           // $notificationq=mysqli_query($con,"INSERT INTO `notification_master` (`n_from`, `n_to`, `b_id`, `id`, `notification`, `n_time`, `status`) VALUES ('{$userdata['u_id']}', 'Student', '{$_SESSION['fb_id']}', 'All', 'New Document Uploaded', CURRENT_TIMESTAMP, '1');");
            if($q)
            {
     ...
     echo "<script>alert('Only PDF File..')</script>";   
 }
    }
    else
    {
            ...
            <div class="content-wrapper">
                <div class="container-fluid">
                    ...
                    <div class="row pt-2 pb-2">
                        <div class="col-sm-9">
                            <h4 class="page-title">Add Documents</h4>
                                ...
                                <div class="card-body">
                                    <form id="personal-info" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group row">
                                            <label for="input-1" class="col-sm-2 col-form-label">Document</label>
                                            <div class="col-sm-10">
                                        ...
                                        <div class="form-group row">
                                            <label for="input-1" class="col-sm-2 col-form-label">Info</label>
                                            <div class="col-sm-10">
