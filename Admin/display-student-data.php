...
FROM
    ...
    INNER JOIN `oms`.`student_master` 
        ...
        if(isset($_GET['sid']))
{
   $d= mysqli_query($con, "update student_master set status=0 where stu_id='{$_GET['sid']}'") or die(mysqli_error($con));
   if($d)
   {
       echo "<script>alert('Delete Successful');window.location='dis-student.php';</script>";
   }
}
 ...
 $i=1;
                        while($row= mysqli_fetch_array($student))
                        {
                            ...
                            echo "<td><a href='display-student-data.php?sid={$row['stu_id']}' onclick='return confirm_delete()'>Detain</a></td>";
                            echo "</tr>";
                            $i++; 
                            }
